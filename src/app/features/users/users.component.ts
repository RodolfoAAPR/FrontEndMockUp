...
export class UsersComponent {
  ...
  openEditModal(transporter: Transporter): void {
    this.editingTransporter = { ...transporter };
    this.showModal = true;
  }
  ...
  saveTransporterEdit(): void {
    const t = this.editingTransporter!;
    if (!t.name.trim() || !t.plate.trim()) return;

    this.transporters = this.transporters.map(item =>
      item.id === t.id
        ? { ...t, name: t.name.trim(), plate: t.plate.trim().toUpperCase() }
        : item
    );
    this.closeModal();
  }
  ...
  deleteTransporter(id: number): void {
    this.transporters = this.transporters.filter(t => t.id !== id);
  }
